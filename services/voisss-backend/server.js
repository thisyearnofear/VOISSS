...
app.post('/api/dubbing/start', upload.single('audio'), async (req, res) => {
  try {
    ...
    if (!response.ok) {
      ...
      // Handle legacy workspace error specifically
      if (errorText.includes('invalid_workspace_type')) {
        return res.status(402).json({
          error: 'ElevenLabs workspace upgrade required',
          message: 'Your ElevenLabs API key is on a legacy workspace that doesn\'t support the dubbing API. Please upgrade your ElevenLabs workspace or contact support.',
...
app.get('/api/dubbing/:dubbingId/status', async (req, res) => {
  try {
    ...
    console.log('Status check request:', { dubbingId });
...
app.get('/api/dubbing/:dubbingId/audio/:targetLanguage', async (req, res) => {
  try {
    ...
    console.log('Audio retrieval request:', { dubbingId, targetLanguage });
