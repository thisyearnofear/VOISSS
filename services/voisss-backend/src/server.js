...
app.use((req, res, next) => {
  res.setHeader(
    ...
    "default-src 'self' *; connect-src 'self' *; media-src 'self' *; img-src 'self' * data:;"
  );
  next();
});
...
app.post("/api/transform", upload.single("audio"), async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${ELEVEN_API_BASE}/speech-to-speech/${voiceId}`,
      {
        method: "POST",
        headers: {
          "xi-api-key": API_KEY,
...
app.post("/api/dubbing/start", upload.single("audio"), async (req, res) => {
  try {
    ...
    const { targetLanguage, sourceLanguage, modelId, preserveBackgroundAudio } =
      req.body;
...
app.post("/api/dubbing/complete", upload.single("audio"), async (req, res) => {
  try {
    const { targetLanguage, sourceLanguage, preserveBackgroundAudio } =
      req.body;
    ...
    if (!startResponse.ok) {
      ...
      console.error(
        "ElevenLabs API Error (complete dubbing start):",
        errorText
      );
      ...
      if (errorText.includes("invalid_workspace_type")) {
        return res.status(402).json({
          ...
          message:
            "Your ElevenLabs API key is on a legacy workspace that doesn't support the dubbing API.",
    ...
    while (status !== "dubbed") {
      ...
      if (elapsed > maxWaitMs) {
        return res.status(408).json({
          ...
          message: `Dubbing took longer than expected (${Math.round(
            elapsed / 1000
          )}s)`,
          dubbingId,
        });
      ...
      const statusResponse = await fetch(
        `${ELEVEN_API_BASE}/dubbing/${dubbingId}`,
        {
          headers: { "xi-api-key": API_KEY },
          timeout: 30000,
        }
      ...
      if (pollCount % 10 === 0) {
        console.log(
          `Complete dubbing progress: ${Math.round(
            elapsed / 1000
