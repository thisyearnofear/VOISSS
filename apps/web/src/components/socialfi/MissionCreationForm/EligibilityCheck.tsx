...
export default function EligibilityCheck({}: EligibilityCheckProps) {
  const {
    isConnected,
  ...
  const {
    balance: voisssBalance,
    tier,
    isLoading: isLoadingVoisss,
    refreshBalance,
    error: voisssError,
  } = useTokenAccess({
    address,
    ...
    return (
      <div className="voisss-card text-center py-8">
        ...
        <p className="text-gray-400">
          You need to connect your wallet to create missions.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="voisss-card text-center py-8">
        ...
        <p className="text-gray-400 mb-4">
          We couldn&apos;t fetch your token balances. This might be due to
          network issues.
        </p>
        <button
          onClick={() => {
  ...
  if (!meetsPapajamsRequirement && !meetsVoisssRequirement) {
    return (
      <div className="voisss-card space-y-4">
        ...
        >
          <div
            className={`w-5 h-5 rounded-full mt-0.5 flex-shrink-0 text-lg ${
          ...
          <div className="text-left flex-1">
            <p
              className={`font-medium ${
                meetsPapajamsRequirement ? "text-green-300" : "text-red-300"
              }`}
            >
              📊 $papajams: Creator Stake (1M minimum)
            </p>
            <p className="text-sm opacity-75 mt-1">
              Ensures you have skin in the game. You&apos;ll receive 70% of all
              rewards from your mission as $papajams.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="text-left flex-1">
            <p
              className={`font-medium ${
                meetsVoisssRequirement ? "text-blue-300" : "text-red-300"
              }`}
            >
              💜 $voisss: Platform Tier (10k+ minimum = Basic tier)
            </p>
            <p className="text-sm opacity-75 mt-1">
              Demonstrates platform commitment. Unlocks creator features and 30%
              platform allocation goes to ecosystem support.
            </p>
        ...
        {!meetsPapajamsRequirement && !meetsVoisssRequirement ? (
          <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-700/50">
            <p className="text-gray-400 text-sm">
              You need to acquire at least one token to create missions. Get
              $papajams or $voisss and try again.
            </p>
          </div>
        ) : null}
      </div>
