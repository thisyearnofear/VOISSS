...
// ─── Seed missions — shown when API returns empty to make the board look active ───

const SEED_MISSIONS: Mission[] = [
  {
    id: "seed-1",
    title: "Voice AI Market Intelligence",
    description: "Automated market research report on voice AI trends, pricing, and competitor analysis. Generated by the Market Intelligence Agent.",
    difficulty: "easy",
    targetDuration: 60,
    language: "en",
  ...
  },
  {
    ...
    language: "en",
    topic: "general",
    tags: ["ai", "society", "future", "conversation"],
    baseReward: "25",
  ...
  },
  {
    ...
    language: "en",
    topic: "culture",
    tags: ["coffee", "culture", "local", "conversation"],
    baseReward: "15",
...
export default function MissionBoard({ onMissionSelect }: MissionBoardProps) {
  ...
  // Merge API missions with seed missions so the board always looks active
  const missions = React.useMemo(() => {
    const validSeeds = SEED_MISSIONS.filter((seed) => {
      if (selectedTopic !== "all" && seed.topic !== selectedTopic) return false;
      if (selectedDifficulty !== "all" && seed.difficulty !== selectedDifficulty) return false;
      return true;
    });
    // Prepend seed missions, deduplicate by id (API data takes precedence)
    const apiIds = new Set(apiMissions.map((m: Mission) => m.id));
    const newSeeds = validSeeds.filter((s) => !apiIds.has(s.id));
    return [...newSeeds, ...apiMissions];
  }, [apiMissions, selectedTopic, selectedDifficulty]);
