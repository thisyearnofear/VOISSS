...
/* ─────────────────────────────────────────────────────────────────────────────
 * TASK ROWS — animated task list with status badges and expandable details
 *
 *     0 ms   rows enter staggered (80 ms apart)
 *   600 ms   row 1 ring sweeps 0 → 66%
 *  1500 ms   row 1 expands — detail steps drop down
 *  3900 ms   row 1 collapses; row 2 flips to Failed + retry
 *  5300 ms   row 2 resolves to Completed
 *
...
function Badge({ tone, children }: { tone: "red" | "green"; children: React.ReactNode }) {
  return (
    <span
      className={`flex size-5.5 shrink-0 items-center justify-center rounded-full text-white ${tone === "red" ? "bg-red-500" : "bg-green-500"}`}
...
export default function TaskRows({ variant = "Cards" }: { variant?: "Cards" | "List" }) {
  ...
  const rows = [
    {
      ...
      amount: "12 suppliers",
      pill: (
        <span className="inline-flex h-5.5 items-center rounded-full bg-green-500/20 px-2 text-[11.5px] font-medium text-green-400">
          Completed
        </span>
      ),
      details: [
    ...
    },
    {
      ...
      pill:
        row2 === "failed" ? (
          <span className="inline-flex h-5.5 items-center gap-1.5 rounded-full bg-red-500/20 px-2 text-[11.5px] font-medium text-red-400" style={{ animation: "fade-in 200ms ease-out both" }}>
            Failed <span style={{ animation: "spin 1.2s linear infinite" }} className="flex">{RetryIcon}</span>
          </span>
        ) : row2 === "done" ? (
          <span className="inline-flex h-5.5 items-center gap-1.5 rounded-full bg-green-500/20 px-2 text-[11.5px] font-medium text-green-400" style={{ animation: "fade-in 200ms ease-out both" }}>
            Completed
          </span>
        ) : null,
      details: [
  ...
  return (
    <div
      className={`flex w-full max-w-110 flex-col ${
        variant === "List"
          ? "gap-0 self-start overflow-hidden rounded-xl bg-gray-900 shadow-md"
          : "min-h-[196px] gap-2"
      }`}
    >
      {rows.map((row, i) => {
        ...
        return (
          <div
            key={row.key}
            className={`self-stretch overflow-hidden transition-[border-radius,background-color] duration-300 hover:bg-gray-800/30 ${
              variant === "List" ? "border-b border-gray-700 last:border-0" : "bg-gray-900 shadow-md"
            }`}
            style={{
          ...
          >
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-white">{row.label}</span>
              <span className="text-[12.5px] text-gray-400 tabular-nums">{row.amount}</span>
              {row.pill}
              <span aria-hidden="true" className="-ml-2 flex size-7 shrink-0 items-center justify-center rounded-full text-gray-500">
                <svg
            ...
            >
              <div className="overflow-hidden">
                <div className="mb-2.5 grid grid-cols-[24px_1fr] gap-2.5 px-2.5">
                  <span aria-hidden className="mx-auto h-full w-px bg-gray-700" />
                  <div className="flex flex-col gap-1.5">
                    {row.details.map((d, j) => (
                      <div
                        key={d.label}
                        className="flex items-center justify-between"
                        style={open ? { animation: `fade-up 300ms cubic-bezier(0.23,1,0.32,1) ${120 + j * 100}ms both` } : undefined}
                      >
                        <span className="text-[12px] text-gray-400">{d.label}</span>
                        <span className="font-mono text-[11.5px] text-gray-500 tabular-nums">{d.meta}</span>
                      </div>
                    ))}
                  </div>
                </div>
