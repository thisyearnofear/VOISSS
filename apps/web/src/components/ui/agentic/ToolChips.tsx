...
/* ─────────────────────────────────────────────────────────────────────────────
 * TOOL CHIPS — compact agent-run trace
 *
 * Renders tool-call rows (think, write, run, read) with inline chips,
 * then file-diff chips that expand on hover to show a diff preview.
 * ───────────────────────────────────────────────────────────────────────────── */
...
const Icons: Record<string, React.ReactNode> = {
  ...
  write: <g fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M17 3a2.8 2.8 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5z" />
  </g>,
  run: <g fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M4 17l6-5-6-5M12 19h8" />
  </g>,
  read: <g fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="min-h-[220px] w-full max-w-80 pb-1">
      ...
      <button
        ...
        className="-mx-1.5 flex w-fit items-center gap-1.5 rounded-lg px-1.5 py-1 text-[12.5px] text-gray-400 transition-colors duration-100 hover:bg-gray-800/50"
      >
        <svg
      ...
      >
        <div className="-mx-1 overflow-hidden px-1.5 pb-1">
              ...
              return (
                <div key={row.label} style={{ animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}>
                  <button
                    ...
                    className="group/row -mx-[3px] flex h-7 w-[calc(100%+6px)] min-w-0 items-center gap-2 rounded-lg px-[3px] text-left transition-colors duration-100 hover:bg-gray-800/50"
                  >
                    <span className="relative flex size-4 shrink-0 items-center justify-center text-gray-400">
                      <svg
                    ...
                    <span className="shrink-0 text-[12.5px] font-medium text-white">{row.label}</span>
                    <span
                      className={`inline-flex h-5.5 min-w-0 flex-1 cursor-pointer items-center truncate rounded-full bg-gray-800 px-1.5 text-[11.5px] text-gray-400 shadow-sm transition-colors duration-100 hover:bg-gray-700 ${row.mono ? "font-mono" : ""}`}
                    >
                      {row.chip}
                    </span>
                  </button>
                  ...
                  >
                    <div className="min-h-0 overflow-hidden">
                      <div className="mt-0.5 mb-1 ml-2 flex flex-col gap-0.5 border-l border-gray-700 py-0.5 pl-3.5">
                        {row.detail.map((line) => (
                          <span
                            key={line.text}
                            className={`truncate text-[11.5px] leading-[1.6] ${row.detailMono ? "font-mono" : ""} ${line.tone === "add" ? "text-green-400" : "text-gray-400"}`}
                          >
                            {line.text}
                          </span>
                        ))}
          ...
          {/* file-diff chips */}
          {step >= total && (
            <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-gray-700 pt-2.5">
              {DIFFS.map((d, i) => (
                <span key={d.file} data-diffchip className="relative">
                  <button
                    type="button"
                    ...
                    className="inline-flex h-7 max-w-full items-center gap-1.5 rounded-full bg-gray-800 px-2 font-mono text-[11.5px] text-white shadow-sm transition-colors duration-100 hover:bg-gray-700"
                    style={{ animation: `pop-in 250ms cubic-bezier(0.23,1,0.32,1) ${i * 80}ms both` }}
                  >
                    <span className="min-w-0 truncate">{d.file}</span>
                    <span className="shrink-0 text-green-400 tabular-nums">+{d.add}</span>
                    {d.del > 0 && <span className="shrink-0 text-red-400 tabular-nums">−{d.del}</span>}
                  </button>
                </span>
              ))}
              <button
                type="button"
                className="inline-flex h-7 items-center rounded-full px-1.5 font-mono text-[11.5px] text-gray-500 underline decoration-transparent underline-offset-2 transition-colors duration-100 hover:text-gray-300 hover:decoration-current"
      ...
      {preview && typeof document !== "undefined" && createPortal(
        <div
          className="fixed z-50 w-72 overflow-hidden rounded-xl bg-gray-900 shadow-xl"
          style={{
            left: preview.x,
            top: preview.top,
        ...
        >
          <div className="flex items-center justify-between border-b border-gray-700 px-2.5 py-1.5 font-mono text-[11px]">
            <span className="min-w-0 truncate text-gray-400">{preview.file}</span>
            <span className="shrink-0 tabular-nums">
              <span className="text-green-400">+{DIFFS.find((diff) => diff.file === preview.file)?.add}</span>
              {(DIFFS.find((diff) => diff.file === preview.file)?.del ?? 0) > 0 && (
                <span className="text-red-400"> −{DIFFS.find((diff) => diff.file === preview.file)?.del}</span>
              )}
            </span>
          </div>
            ...
            {(DIFF_LINES[preview.file] ?? []).map((line, index) => (
              <div
                ...
                className={`flex gap-2 px-2.5 whitespace-pre ${
                  line.tone === "add" ? "bg-green-900/30 text-green-400"
                    : line.tone === "del" ? "bg-red-900/30 text-red-400"
                    : "text-gray-400"
                }`}
              >
