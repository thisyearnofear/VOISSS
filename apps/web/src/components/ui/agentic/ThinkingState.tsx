...
const TONES = ["bg-blue-500", "bg-orange-500", "bg-green-500"];
      ...
      >
        ...
        <span role="status" className="contents">
          {working ? (
            ...
            <span
              className="text-[13px] font-medium whitespace-nowrap text-gray-400"
      ...
      >
        <div className="overflow-hidden">
          <div className="relative mt-1 ml-[5px] pl-4">
            <span
              aria-hidden
              className="absolute left-[3px] w-px bg-gray-700"
            ...
            <div ref={traceRef} className="flex flex-col gap-1 py-1">
              {v.query && (
                <div className="flex h-6 items-center gap-2 px-1.5" style={{ animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}>
                  ...
                  <span className="text-[12.5px] text-gray-400">{v.query}</span>
                </div>
              )}
                  ...
                  <>
                    ...
                    {variant === "Steps" && (
                      i < visible - 1 || !working ? (
                        ...
                        <span className="size-3 shrink-0 rounded-full border-[1.5px] border-gray-600 border-t-purple-400" style={{ animation: "spin 700ms linear infinite" }} />
                      )
                    )}
                    <span className={`min-w-0 truncate text-[12.5px] ${variant === "Reasoning" ? "whitespace-normal leading-relaxed text-gray-400" : "font-medium text-white"} ${variant === "Search" ? "decoration-purple-400/50 underline decoration-[1px] underline-offset-2" : ""}`}>
                      {row.primary}
                    </span>
                    {row.secondary && (
                    ...
                    {row.add !== undefined && (
                      <span className="shrink-0 font-mono text-[11px] tabular-nums">
                        <span className="text-green-400">+{row.add}</span>{" "}
                        <span className="text-red-400">−{row.del}</span>
                      </span>
                    )}
                  </>
                );
                ...
                if (variant === "Search") {
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`${rowClass} transition-colors duration-150 hover:bg-gray-800/50`}
                      style={animation}
                    >
                      {content}
                    </a>
                ...
                if (variant === "Coding") {
                  ...
                  return (
                    <button
                      ...
                      className={`${rowClass} transition-colors duration-150 ${selected ? "bg-gray-800/30" : "hover:bg-gray-800/50"}`}
                      style={animation}
                    >
                      {content}
                    </button>
              ...
              {variant === "Search" && stage >= 3 && (
                <span className="text-[12px] text-gray-500" style={{ animation: "fade-in 300ms ease-out both" }}>
                  +7 more
                </span>
              )}
            </div>
