...
import {
  ...
  getTierForBalance,
  getTokenExplorerUrl,
} from "@voisss/shared/config/tokenAccess";
...
async function fetchBalanceWithRetry(
  ...
  for (const rpcUrl of RPC_PROVIDERS) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[token-balance] Attempting RPC: ${rpcUrl} (attempt ${attempt + 1})`
        );
        ...
        console.log(
          `[token-balance] Successfully fetched balance: ${balance.toString()}`
        );
        return balance;
      } catch (error) {
        ...
        fallbackOptions: [
          {
            ...
            description:
              "View your balance directly on the blockchain explorer",
