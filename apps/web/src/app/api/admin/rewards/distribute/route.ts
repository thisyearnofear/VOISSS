...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !body.submissionId ||
      !body.walletAddress ||
      body.papajamsAmount === undefined ||
      body.voisssAmount === undefined
    ) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: submissionId, walletAddress, papajamsAmount, voisssAmount",
        },
        { status: 400 }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to fetch distributions";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
