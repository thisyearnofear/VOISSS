...
import { Play, Pause, Share2, Heart, Download, User } from 'lucide-react';
...
export default function VoisssMiniAppPlayer() {
  ...
  useEffect(() => {
    try {
      ...
      const payload = (sdk as any).getInitialPayload?.() || (window as any).__MINIAPP_PAYLOAD__;
      ...
      const user = (window as any).__MINIAPP_USER__;
      if (user) {
        setUserContext(user);
      }
  ...
  const handleHeart = async () => {
    try {
      if ((sdk as any).like) {
        await (sdk as any).like(recordingId);
      }
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{
          ...
          {formatDuration(currentTime)} / {formatDuration(mockRecording.duration)}
        </div>
      ...
      <div style={{
        ...
        <button
          onClick={() => {
            if ((sdk as any).openUrl) {
              (sdk as any).openUrl('https://voisss.app');
            } else {
