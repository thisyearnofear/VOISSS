...
export class ClientAIService implements AIServiceClient {
  ...
  async listVoices(): Promise<VoiceInfo[]> {
    ...
    try {
      const res = await fetch(
        `${this.config.apiBaseUrl}/elevenlabs/list-voices`,
        {
          method: "POST",
  ...
  async transformVoice(audioBlob: Blob, voiceId: string): Promise<Blob> {
    try {
      ...
      const res = await fetch(
        `${this.config.apiBaseUrl}/elevenlabs/transform-voice`,
        {
          method: "POST",
          body: form,
        }
  ...
  async getSupportedLanguages(): Promise<LanguageInfo[]> {
    ...
    try {
      const res = await fetch(
        `${this.config.apiBaseUrl}/elevenlabs/dub-audio`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const enhancedLanguages: LanguageInfo[] = rawLanguages.map(
        (lang: any) => {
